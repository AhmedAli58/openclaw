...
- CLI/Skills: add built-in managed skills commands (`skills search/install/list/update/remove`) backed by the `openclaw/skills` GitHub registry with cached index fallback, atomic installs/updates, validation, and `self-improving-agent --init` workspace scaffolding.
- Doctor/Skills: add high-value skills recommendations (`self-improving-agent`, `github`, `summarize`) with direct install guidance for `openclaw skills install self-improving-agent --init`.
